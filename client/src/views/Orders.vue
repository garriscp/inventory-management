<template>
  <div class="orders">
    ...
    <div v-else>
      ...
      <div class="card" v-if="submittedOrders.length > 0">
        <div class="card-header">
          <h3 class="card-title">{{ t('orders.submittedRestocking') }} ({{ submittedOrders.length }})</h3>
          <span class="badge info" style="font-size:0.75rem;">{{ t('orders.inSession') }}</span>
        </div>
...
export default {
  ...
  setup() {
    ...
    const submittedOrders = computed(() =>
      orders.value.filter(o => o.order_number && o.order_number.startsWith('RST-'))
    )
